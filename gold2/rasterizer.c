...
{
  // START CODE HERE
  int net_shift = r_shift - ss_w_lg2;
  return val >> net_shift << net_shift;
  // END CODE HERE
}
...
BoundingBox get_bounding_box(Triangle triangle, Screen screen, Config config)
{
  ...
  int ll_x = min(triangle.v[0].x, min(triangle.v[1].x, triangle.v[2].x));
  // iterate over remaining vertices
  int ur_x = max(triangle.v[0].x, max(triangle.v[1].x, triangle.v[2].x));
  int ll_y = min(triangle.v[0].y, min(triangle.v[1].y, triangle.v[2].y));
  int ur_y = max(triangle.v[0].y, max(triangle.v[1].y, triangle.v[2].y));
  ...
  bbox.valid = ((ll_x > ur_x) || ll_y > ur_y) ? false : true;
